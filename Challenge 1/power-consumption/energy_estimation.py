...
tx_spikes = [d for d in sender_data if d >= 625]

avg_power_states["sender.csv"] = {
    "tx_baseline_mW": avg(tx_baseline),
    "tx_spike_mW": avg(tx_spikes),
}
...
phases = [
    ...
    ("sender_spike", r"sender_spike_us: (\d+)", "sender.csv",      "tx_spike_mW",    US_TO_S),
...
p_tx_spike, p_tx_baseline = sender["tx_spike_mW"], sender["tx_baseline_mW"]
sender_powers = [p_tx_spike, p_tx_baseline] * (num_cycles - 1)
...
plot_step_power_vs_time(
    deep_sleep_times,
    deep_sleep_powers,
    "Power Consumption (Deep-Sleep Cycle)",
...
plot_step_power_vs_time(
    sensor_read_times,
    sensor_powers,
    "Power Consumption (Idle - Sensor Read)",
